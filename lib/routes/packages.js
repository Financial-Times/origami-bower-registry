...
module.exports = app => {
	...
	app.post('/packages/refresh', bodyParser.raw({ type: 'application/x-www-form-urlencoded' }), (request, response, next) => {
		try {
			if (verifyGithubWebhook(request.get('X-Hub-Signature'), request.body, Buffer.from(app.ft.options.githubSecret))) {
				app.ft.packageData.loadFromGitHub();
			}
			response.send('OK');
		} catch (error) {
			next(httpError(403));
